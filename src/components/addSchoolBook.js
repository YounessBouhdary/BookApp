...
class AddSchoolBook extends React.Component {
    state = {
        livre: {
            titreLivre: "",
            matiere: "",
    ...
    subjects = ["Mathématiques", "Physique et Chimie", "Science Vie et de terre",
    ...
    levels = ["1ère Année Primaire", "2ème Année Primaire", "3ème Année Primaire",
                ...
                Axios.post('https://kotobima.herokuapp.com/upload', fd)
                    .then(res => {
                        ...
                        if (this.state.errors.recaptcha === "") {
                            Axios.post('https://kotobima.herokuapp.com/addBook_school', this.state.livre);
                ...
                <div className="allParts">
                    {this.state.stepsArray[0] ?
                        <div className="firstPart">
                            ...
                            <div className="label_input"><label>Niveau: </label>
                                <select defaultValue="" className="Niveau">
                                    <option hidden value="">Niveau scolaire</option>
                                    {this.levels.map((item, idx) =>
                                <option key={idx} value={(item.toLowerCase())}>{item}</option>)}
                                
                                </select></div>
                            <div className="label_input"><label>Matière: </label>
                                <select defaultValue="" className="matiere">
                                    <option hidden value="">Matière: </option>
                                    {this.subjects.map((item, idx) =>
                                        <option key={idx} value={(item.toLowerCase())}>{item}</option>)}
